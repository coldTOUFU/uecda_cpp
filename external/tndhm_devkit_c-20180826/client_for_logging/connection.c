...
extern FILE *g_read_fptr;
extern FILE *g_write_fptr;
int logging_io;
int my_turn_cnt;
...
int closeSocket() { return close(g_sockfd); }
...
int receiveCards(int cards[8][15]) {
  ...
  if (logging_io) {
    for (int i = 0; i < 8; i++) {
      for (int j = 0; j < 15; j++) {
        fprintf(g_read_fptr, "%d", cards[i][j]);
        if (j < 14) {
          fprintf(g_read_fptr, " ");
        }
      }
...
int sendCards(int cards[8][15]) {
  ...
  if (logging_io) {
    fprintf(g_write_fptr, "%d\n", my_turn_cnt++);
    for (int i = 0; i < 8; i++) {
      for (int j = 0; j < 15; j++) {
        fprintf(g_write_fptr, "%d", cards[i][j]);
        if (j < 14) {
          fprintf(g_write_fptr, " ");
        }
      }
      fprintf(g_write_fptr, "\n");
    }
  }
  if ((sendTable(cards)) == -1) { //�J�[�h�𑗐M��
                                  //���s���̓��b�Z�[�W��\��
...
static int openSocket(const char addr[], uint16_t port_num) {
  ...
  /* �T�[�o�ɃR�l�N�g���� */
  if (connect(g_sockfd, (struct sockaddr *)&g_client_addr,
              sizeof(g_client_addr)) == 0) {
    return 0;
  }
  return -1;
}
